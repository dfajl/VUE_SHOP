export default {
    ...
    SET_CART: (state, product)=> {
        if(state.cart.length) { // есть ли корзина (т.е., не пустая ли она?)
            let isProductExists = false; 
            // это флаг, определяющий наличие определенного товара в корзине

            let mapFunction = state.cart.map( (item)=> { // просто создал переменную, чтобы посмотреть, что возвращает метод массива map()
